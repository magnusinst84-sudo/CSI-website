...
export default function Partners() {
  ...
  // We copy the list before reversing to avoid modifying the original list multiple times on re-renders
  const reversedPartners = [...partnersList].reverse();
      ...
      >
        ...
        <div className="marquee mb-6">
          <div className="marquee-track">
            {[...partnersList, ...partnersList].map((name, i) => (
              <div
                key={`${name}-${i}`}
        ...
        <div className="marquee">
          ...
          >
            {[...reversedPartners, ...partnersList].map((name, i) => (
              <div
                key={`r-${name}-${i}`}
                className="flex items-center gap-3 px-8 py-4 glass rounded-xl transition-all duration-300 hover:bg-white/5 hover:scale-105 cursor-default group flex-shrink-0"
              >
